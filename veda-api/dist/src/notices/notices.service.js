...
let NoticesService = class NoticesService {
    ...
    async create(createDto, user) {
        if (!user.hospitalId)
            throw new common_1.BadRequestException('Hospital ID required');
    ...
    async findAll(user) {
        const query = { hospitalId: user.hospitalId };
    ...
    async updateStatus(id, status, hospitalId) {
        const notice = await this.noticeModel.findOneAndUpdate({ _id: id, hospitalId }, { status }, { new: true, returnDocument: 'after' }).exec();
        if (!notice)
            throw new common_1.NotFoundException('Notice not found');
        return notice;
    }
    async remove(id, hospitalId) {
